...
function create_post_type_tutor() {
    register_post_type('english-tutor', array(
        'labels' => array(
            'name' => 'Gia sư'
        ),
        'public' => true,
    ...
    register_taxonomy('tutor-cat', 'english-tutor', array(
        'labels' => array(
            'name' => 'Thể loại Gia Sư',
        ),
        'public' => true,
        'hierarchical' => true,
        'show_admin_column' => true,
        'rewirte' => array('slug' => 'tutor-cat'),
        'query_var' => true
    ));
   
}

add_action('init', 'create_post_type_tutor');

//add field to post
add_action('add_meta_boxes', 'add_my_tutor_field');

function add_my_tutor_field() {
    add_meta_box('info-tutor', 'Thông tin Gia sư', 'show_tutor_box', 'english-tutor', 'normal', 'high', array());
}
...
function show_tutor_box() {
    ...
    <input type="hidden" name="english-tutor-information" />
    <?php
}

//save contact
add_action('save_post', 'update_tutor_post');

function update_tutor_post($post_id) {
    global $tutor_fields;
    ...
    if (isset($_REQUEST['post_type']) && $_REQUEST['post_type'] == 'teacher' && isset($_POST['english-tutor-information'])) {
        foreach ($tutor_fields as $box) {
            update_post_meta($post_id, $box['name'], $_POST[$box['name']]);
        }
    } else {
        if (isset($_POST['english-tutor-information'])) {
            foreach ($tutor_fields as $box) {
